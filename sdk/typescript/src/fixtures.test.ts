...
test("RFC 3339 date-time corpus — createdAt and approvedAt (spec §4, §7)", async () => {
  const { validate } = await import("./validate.ts");
  const mismatches: string[] = [];
  for (const { value, valid, why } of load("timestamps.json")) {
    const doc = structuredClone(load("validation-messages.json")[0].document);
    doc.provenance.createdAt = value;
    doc.approvals = [{ fingerprint: "sha256:aaaa", approvedBy: "r", approvedAt: value }];
    const errors = validate(doc).errors.map((e: { path: string }) => e.path);
    const expected = valid ? [] : ["$.provenance.createdAt", "$.approvals[0].approvedAt"];
    if (JSON.stringify(errors) !== JSON.stringify(expected)) {
      mismatches.push(`${JSON.stringify(value)} (${why}): expected ${JSON.stringify(expected)}, got ${JSON.stringify(errors)}`);
    }
  }
  assert.equal(mismatches.length, 0, mismatches.join("\n"));
});
